...
const getMonitorTypeIcon = (type?: Monitor['type']) => {
  if (type === 'website')
    return <LinkIcon sx={{ fontSize: '1rem', verticalAlign: 'middle', mr: 0.5 }} />;
  if (type === 'ping')
    return <PingIcon sx={{ fontSize: '1rem', verticalAlign: 'middle', mr: 0.5 }} />;
  return null;
};
                  ...
                  >
                    <CardContent sx={{ flexGrow: 1, pb: 1 }}>
                      <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                        ...
                        <Tooltip
                          title={
                            monitor.latestStatus?.status === true
                              ? 'Online'
                              : monitor.latestStatus?.status === false
                                ? 'Offline'
                                : 'Status Unknown'
                          }
                        >
                          <Box sx={{ ml: 1, mt: '4px' }}>
                            {getStatusIcon(monitor.latestStatus?.status)}
                          </Box>
                        </Tooltip>
                      </Stack>
